...
export function SystemSignOffTab({ plannerState, projectId, canManageAccessLink = false, externalToken, initialRecord, projectName, workspaceLogoUrl, electricianDetails }: SystemSignOffTabProps) {
  ...
  function exportPdf() {
    ...
    popup.document.write(`<html><head><title>${escapeHtml(projectName ?? record?.project_name ?? "System Sign-Off")}</title><style>
      @page portraitForm{size:A4 portrait;margin:8mm}
      @page landscapeForm{size:A4 landscape;margin:8mm}
      *{box-sizing:border-box}body{margin:0;font-family:Arial,sans-serif;color:#111;background:#fff}
      .pdf-page{width:100%;background:#fff;break-after:page}.pdf-page:last-child{break-after:auto}
      .portrait-page{page:portraitForm}.landscape-page{page:landscapeForm}
      ...
      .certificate-logo{width:23mm;height:17mm;object-fit:contain;object-position:left top}.certificate-logo-space{display:block;width:23mm}
      .certificate-copy{text-align:center}.certificate-copy h1{margin:0 0 2mm;font-size:17px}.certificate-copy .subtitle{margin:0 0 2mm;font-size:8px;font-weight:700;line-height:1.3}.certificate-copy .description{margin:0;font-size:7.5px;line-height:1.3}
      .draft{margin:0;padding:1.5mm 2mm;border:1px solid #9a6700;background:#fff7e6;color:#92400e;font-size:8px;font-weight:700}
      .part-title{margin:0;padding:1.4mm 2mm;border:1px solid #333;background:#ddd;font-size:10px}
      .field-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));border-left:1px solid #333}.field{min-height:12mm;padding:1.4mm 1.8mm;border-right:1px solid #333;border-bottom:1px solid #333;font-size:7.5px;line-height:1.25;overflow-wrap:anywhere}.field.full{grid-column:1/-1}.field strong{display:block;margin-bottom:.8mm;font-size:7.5px}.value{min-height:4mm;white-space:pre-wrap}.checks{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));border-left:1px solid #333}.check-row{padding:1.6mm;border-right:1px solid #333;border-bottom:1px solid #333;font-size:7.5px}.check-mark{display:inline-block;width:3mm;height:3mm;margin-right:1.2mm;border:1px solid #333;text-align:center;line-height:2.5mm;font-weight:700}
      table{width:100%;border-collapse:collapse;table-layout:fixed}th,td{padding:1.2mm;border:1px solid #333;text-align:left;vertical-align:top;font-size:6.5px;line-height:1.2;overflow-wrap:anywhere}th{background:#eee;font-weight:700}.g2-table{margin:0}.g2-table th,.g2-table td{font-size:5.6px;padding:.9mm}.instrument-table th,.instrument-table td{font-size:7px}
      .g2-meta{display:grid;grid-template-columns:1fr 1fr;border-left:1px solid #333}.g2-meta .field{min-height:9mm}
      .declaration{padding:2mm;border:1px solid #333;border-top:0;font-size:7.5px;line-height:1.3}.completion-note{margin-top:2mm;font-size:7px;color:#555}
      @media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact}}
    </style></head><body>${documentForPrint()}<script>window.onload=()=>{const images=[...document.images];Promise.all(images.map((image)=>image.complete?Promise.resolve():new Promise((resolve)=>{image.onload=resolve;image.onerror=resolve;}))).then(()=>window.print());}</script></body></html>`);
  ...
  function documentForPrint() {
    const value=(label:string,content:unknown,full=false)=>`<div class="field${full?" full":""}"><strong>${escapeHtml(label)}</strong><div class="value">${escapeHtml(content)}</div></div>`;
    const checked=(label:string,isChecked:boolean)=>`<div class="check-row"><span class="check-mark">${isChecked?"✓":""}</span>${escapeHtml(label)}</div>`;
    const header=(title:string,form:"G1"|"G2"|"G3",description:string)=>`<div class="certificate-head">${workspaceLogoUrl?`<img class="certificate-logo" src="${escapeHtml(workspaceLogoUrl)}" alt="Company logo"/>`:'<span class="certificate-logo-space"></span>'}<div class="certificate-copy"><h1>${escapeHtml(title)}</h1><p class="subtitle">(Form ${form} for use with BS 7909, Code of practice for temporary electrical systems for entertainment and related purposes)</p><p class="description">${escapeHtml(description)}</p></div><span class="certificate-logo-space"></span></div>`;
    const draft=record?.status==="submitted"?"":'<p class="draft">DRAFT - NOT SUBMITTED</p>';
    const scoped=(form:SignOffG1Certificate,key:string,automatic:string)=>document.autoFieldOverrides[`${form.id}.${key}`]??automatic;
    const sourceFor=(form:SignOffG1Certificate)=>form.sourceId?plannerState.sources.find((source)=>source.id===form.sourceId&&!source.auto):undefined;
    const demandFor=(form:SignOffG1Certificate)=>{
      const source=sourceFor(form);
      if(!source)return "";
      const va=plannerState.distros.filter((distro)=>distro.sourceId===source.id).reduce((sum,distro)=>sum+distroApparentVa(distro.id),0);
      const voltage=source.phaseType==="Three-Phase"?(plannerState.advancedElectrical?.nominalThreePhaseVoltage??400):(plannerState.advancedElectrical?.nominalSinglePhaseVoltage??230);
      const amps=source.phaseType==="Three-Phase"?va/(Math.sqrt(3)*voltage):va/voltage;
      return form.maximumDemandUnit==="A"?(va?amps.toFixed(2):""):(va?(va/1000).toFixed(2):"");
    };
    const g1Pages=document.g1Forms.map((form)=>{
      const source=sourceFor(form);
      const automatic=(content:string)=>form.sourceId===null?"":content;
      const venue=[document.sharedInformation.venue,document.sharedInformation.venueLocation].filter(Boolean).join(", ");
      const sourceProtection=[form.sourceOvercurrentSelected?`CB/RCBO/fuse ${form.sourceDeviceRating} A ${form.sourceDeviceType}`:"",form.sourceResidualSelected?`RCD/RCBO IΔn ${form.sourceRcdRatingMa} mA, delay ${form.sourceRcdDelayMs} ms`:""].filter(Boolean).join("; ");
      const isuProtection=[form.isuOvercurrentSelected?`CB/RCBO/fuse ${form.isuDeviceRating} A ${form.isuDeviceType}`:"",form.isuResidualSelected?`RCD/RCBO IΔn ${form.isuRcdRatingMa} mA, delay ${form.isuRcdDelayMs} ms`:""].filter(Boolean).join("; ");
      const completion=completionInfo(document.completion.g1[form.id]);
      return `<section class="pdf-page portrait-page">${header("Completion Certificate","G1","This Certificate, showing the results of inspections and tests carried out on the temporary distribution described, should be handed to the event manager. A copy should be available for the owner of the electrical supply which feeds the temporary system. One certificate should be prepared for each electrically separate temporary distribution. This document is not valid without a completed Schedule of Test Results.")}${draft}<div class="field-grid">${value("Certificate name",form.title)}${value("Certificate Reference No.",scoped(form,"g1.reference",automatic(form.certificateReference||record?.certificate_reference||"")))}</div><h2 class="part-title">Part 1: Description of the activity being covered and supply characteristics</h2><div class="field-grid">${value("1. Event",scoped(form,"g1.event",automatic(document.sharedInformation.eventName)))}${value("2. Location or venue",scoped(form,"g1.venue",automatic(venue)))}${value("3. Subsection of a larger system - details",scoped(form,"g1.subsection",form.subsectionDetails))}${value("4. Supply and maximum demand",`${form.supplyPhase==="three"?"Three-phase":form.supplyPhase==="single"?"Single phase":""}${form.supplyPhase&&demandFor(form)?"; ":""}${scoped(form,"g1.maximumDemand",demandFor(form))}${demandFor(form)||document.autoFieldOverrides[`${form.id}.g1.maximumDemand`]?` ${form.maximumDemandUnit}`:""}`)}${value("5. Date of inspection and test",scoped(form,"g1.inspectionDate",automatic(document.sharedInformation.eventDate)),true)}</div><h2 class="part-title">Part 2: System details of supply used</h2><div class="field-grid">${value("6. Source of supply used",`${form.sourceSupplyKind==="generator"?"Generator":form.sourceSupplyKind==="installed"?"Installed supply":""}${form.sourceLocation?`; ${form.sourceLocation}`:""}`)}${value("7. Supply earthing arrangements",form.earthingArrangement)}${value("8. Protective devices at source of supply",sourceProtection)}${value("9. Additional earthing arrangements",`${form.earthElectrodesDeployed==="yes"?"Earth electrodes deployed":"No earth electrodes recorded"}${form.earthElectrodeDetails?`; ${form.earthElectrodeDetails}`:""}`)}${value("10. Interconnection of earthing systems",`${form.earthingSystemsInterconnected==="yes"?"Deliberate connections made":"No deliberate connections recorded"}${form.interconnectionDetails?`; ${form.interconnectionDetails}`:""}`)}${value("11. Protective devices in the ISU",isuProtection)}${value("12. Final circuit details and tests", "Final circuit details and tests should be shown on a G2 - Schedule of Test Results, where appropriate.",true)}${value("13. Deviations or other significant information",form.deviations,true)}</div><h2 class="part-title">Part 3: Essential inspection and tests</h2><div class="checks">${checked("14. Visual inspection satisfactory",form.visualInspectionSatisfactory)}${checked("15. Polarity throughout satisfactory",form.polaritySatisfactory)}${checked("16. Earth fault loop Z throughout satisfactory",form.earthFaultLoopSatisfactory)}${checked("17. RCD test buttons satisfactory",form.rcdButtonsSatisfactory)}${checked("18. Evidence of formal inspection and test provided and satisfactory for electrical equipment",form.equipmentEvidenceSatisfactory)}${value("19. Earth loop impedance of the supply (Ω)",form.supplyEarthLoopImpedanceOhms)}${value("20. Planned duration of this system",scoped(form,"g1.duration",automatic(document.sharedInformation.operatingPeriod)))}${value("21. Date to re-inspect and re-test this system",form.reinspectionDate)}</div><h2 class="part-title">Part 4: Declaration</h2><div class="declaration">I certify that the temporary electrical distribution system described above has been set-up in accordance with the recommendations of BS 7909:2011 and inspection and testing has been completed. To the best of my knowledge and belief, the system is safe and suitable for the intended purpose.</div><div class="field-grid">${value("Name",scoped(form,"g1.name",automatic(document.sharedInformation.responsiblePerson)))}${value("Responsibility on event",scoped(form,"g1.responsibility",automatic(document.sharedInformation.responsibility)))}${value("For and on behalf of",scoped(form,"g1.onBehalf",automatic(document.sharedInformation.onBehalfOf)))}${value("Date",scoped(form,"g1.date",automatic(document.sharedInformation.signOffDate)))}${value("Signature / typed confirmation",form.signature)}${value("Declaration confirmed",form.declarationConfirmed?"Yes":"No")}</div>${completion?`<p class="completion-note">Completed by ${escapeHtml(completion.completedBy)} - ${escapeHtml(new Date(completion.completedAt).toLocaleString())}</p>`:""}</section>`;
    }).join("");
    const plannedRows=selectedCircuits.map((circuit)=>{const result=document.g2Results[circuit.id]??emptyResult();const residual=residualValues(circuit.output);return [overrideValue(`circuit.${circuit.id}.description`,`${displayDistroName(circuit.distro)} / ${circuit.label}`),circuit.classification==="Final"?"Yes":"No",overrideValue(`circuit.${circuit.id}.device`,deviceText(circuit.output)),overrideValue(`circuit.${circuit.id}.rcdMode`,residual.mode),overrideValue(`circuit.${circuit.id}.delay`,residual.delay),overrideValue(`circuit.${circuit.id}.rcdRating`,residual.rating),result.rcdTestMethod,result.polarity,result.phaseSequence,result.measuredZsOhms,result.prospectiveFaultCurrentKa,result.comments];});
    const manualRows=document.manualG2Circuits.map((circuit)=>{const result=document.g2Results[circuit.id]??emptyResult();return [circuit.circuitDetails,circuit.finalCircuit?"Yes":"No",circuit.deviceTypeAndRating,circuit.rcdMode,circuit.rcdDelayMs,circuit.residualRatingMa,result.rcdTestMethod,result.polarity,result.phaseSequence,result.measuredZsOhms,result.prospectiveFaultCurrentKa,result.comments];});
    const tableRows=[...plannedRows,...manualRows].map((row)=>`<tr>${row.map((cell)=>`<td>${escapeHtml(cell)}</td>`).join("")}</tr>`).join("")||`<tr>${g2Headings.map(()=>"<td></td>").join("")}</tr>`;
    const instrumentRows=document.instruments.map((instrument)=>`<tr><td>${escapeHtml(instrument.function)}</td><td>${escapeHtml(instrument.manufacturer)}</td><td>${escapeHtml(instrument.model)}</td><td>${escapeHtml(instrument.serialNumber)}</td><td>${escapeHtml(instrument.calibrationExpiry)}</td></tr>`).join("")||'<tr><td></td><td></td><td></td><td></td><td></td></tr>';
    const g2Page=`<section class="pdf-page landscape-page">${header("Schedule of Test Results","G2","This schedule has to be accompanied by a valid Completion Certificate.")}${draft}<div class="g2-meta">${value("Completion Certificate reference",document.g2.completionCertificateReference||certificateReference)}${value("Date",document.g2.scheduleDate||document.sharedInformation.signOffDate)}</div><table class="g2-table"><thead><tr>${g2Headings.map((heading)=>`<th>${escapeHtml(heading)}</th>`).join("")}</tr></thead><tbody>${tableRows}</tbody></table><h2 class="part-title">Test instrument details</h2><div class="field-grid">${value("Combined instrument",document.g2.combinedInstrument?"Yes":"No",true)}</div><table class="instrument-table"><thead><tr><th>Function</th><th>Manufacturer</th><th>Model</th><th>Serial number</th><th>Calibration expiry</th></tr></thead><tbody>${instrumentRows}</tbody></table></section>`;
    const g3Page=document.g3.notApplicable?"":`<section class="pdf-page portrait-page">${header("Confirmation of Electrical Completion","G3","This Certificate summarises the individual completion certificates for each subsection of the temporary distribution described. It confirms that the temporary electrical system and its sub-systems associated with the event detailed below have been set-up, inspected and tested appropriately to ensure that they are safe and suitable for use. This form should be handed to the event manager. A copy should be available for the owner of the electrical supply which feeds the temporary system. It should be accompanied by the number of Completion Certificates and Schedules of Test Results as stated in row 4.")}${draft}<h2 class="part-title">Part 1: Details of event</h2><div class="field-grid">${value("1. Event",overrideValue("g3.event",document.sharedInformation.eventName))}${value("2. Location or venue",overrideValue("g3.venue",[document.sharedInformation.venue,document.sharedInformation.venueLocation].filter(Boolean).join(", ")))}${value("3. Start date",overrideValue("g3.start",document.sharedInformation.eventDate))}${value("Planned removal date",overrideValue("g3.removal",document.sharedInformation.plannedRemovalDate))}${value("4. Number of certificates attached",overrideValue("g3.certificateCount",String(document.g3.sections.filter((section)=>section.certificateReference).length)),true)}</div><h2 class="part-title">Part 2: Schedule of sections</h2><table><thead><tr><th>Sub-system</th><th>Person Responsible</th><th>Organisation</th><th>Certificate reference</th></tr></thead><tbody>${document.g3.sections.map((section)=>`<tr><td>${escapeHtml(section.subsystem)}</td><td>${escapeHtml(section.responsiblePerson)}</td><td>${escapeHtml(section.organisation)}</td><td>${escapeHtml(section.certificateReference)}</td></tr>`).join("")||'<tr><td></td><td></td><td></td><td></td></tr>'}</tbody></table><h2 class="part-title">Part 3: Confirmation</h2><div class="declaration">As the Senior Person Responsible, I confirm that the temporary electrical systems listed above are safe and suitable for the purposes required by this event.</div><div class="field-grid">${value("Print name",overrideValue("g3.name",document.sharedInformation.responsiblePerson))}${value("For and on behalf of",overrideValue("g3.onBehalf",document.sharedInformation.onBehalfOf))}${value("Date",overrideValue("g3.date",document.sharedInformation.signOffDate))}${value("Signature / typed confirmation",document.g3.signature)}${value("Declaration confirmed",document.g3.declarationConfirmed?"Yes":"No",true)}</div></section>`;
    return `${g1Pages}${g2Page}${g3Page}`;
  }
  ...
  return <section className={`system-signoff${activeFormCompleted?" form-complete":""}`} style={styles.page}>
    ...
    {submitModalOpen&&<div style={styles.modalBackdrop}><section style={styles.modal}><h3>Submit Sign-Off?</h3><p>Pressing submit will lock this revision and send a PDF copy of every G1 certificate, G2 Schedule of Test Results and {document.g3.notApplicable?"no G3 form because it is marked not applicable":"the G3 Confirmation of Electrical Completion"} to <strong>{externalToken?"the project account holder's email":accountEmail||"the project account holder's email"}</strong>.</p><p>The email will be sent from <strong>hello@lvapowerplanner.com</strong> with the completed documentation attached.</p><p><strong>{selectedCircuits.length}</strong> circuits are included; <strong>{selectedCircuits.filter((c)=>((document.g2Results[c.id]?.result??"not-tested") === "not-tested")).length}</strong> remain not tested.</p><div style={styles.modalActions}><button style={styles.primaryButton} disabled={submitting} onClick={()=>void submitSignOff()}>{submitting?"Generating and sending PDF…":"Submit, lock and email PDF"}</button><button style={styles.secondaryButton} disabled={submitting} onClick={()=>setSubmitModalOpen(false)}>Cancel</button></div></section></div>}
