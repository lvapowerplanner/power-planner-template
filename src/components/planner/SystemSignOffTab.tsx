...
export function SystemSignOffTab({ plannerState, projectId, canManageAccessLink = false, externalToken, initialRecord, projectName, electricianDetails }: SystemSignOffTabProps) {
  ...
  const completingPerson = electricianDetails?.name || document.sharedInformation.responsiblePerson || "Project user";
  ...
  return <section className={`system-signoff${activeFormCompleted?" form-complete":""}`} style={styles.page}>
    ...
    <style>{`.system-signoff h2,.system-signoff h3,.system-signoff p{margin-top:0}.system-signoff table th,.system-signoff table td{text-align:left;vertical-align:top;padding:8px;border:1px solid #98a2b3}.system-signoff table th{background:#f2f4f7;font-weight:700;white-space:normal}.system-signoff table input,.system-signoff table select{width:100%;min-height:34px;box-sizing:border-box;border:1px solid #cbd5e1;border-radius:5px;padding:5px 7px;background:#fff}.system-signoff button:disabled,.system-signoff input:disabled,.system-signoff select:disabled,.system-signoff textarea:disabled{cursor:not-allowed;opacity:.7}`}</style>
    ...
    <nav style={styles.subTabs}>{([['shared','Information'],['g1','G1 - Completion Certificate'],['g2','G2 - Schedule of Test Results'],['g3','G3 - Confirmation of Electrical Completion'],['circuits','Include Circuits']] as [SubTab,string][]).map(([id,label]) => <button key={id} style={subTabStyle(id)} onMouseEnter={() => setHoveredSubTab(id)} onMouseLeave={() => setHoveredSubTab(null)} onClick={(event) => { setActiveSubTab(id); event.currentTarget.blur(); }}>{label}</button>)}</nav>

    {activeSubTab === "shared" && <section style={styles.informationPage}>
      <h3 style={styles.sectionTitle}>Information</h3>
      <p style={styles.muted}>Shared details, electricians and sign-off progress.</p>
    ...
    {activeSubTab === "circuits" && <section style={styles.card}>
      ...
      {plannerState.distros.map((distro) => {
        ...
        return <div key={distro.id} style={styles.distroGroup}>
          <button style={styles.distroHeader} onClick={()=>setCollapsedDistroIds((current)=>collapsed?current.filter((id)=>id!==distro.id):[...current,distro.id])}><span><strong>{displayDistroName(distro)}</strong><small>{rows.length} populated circuit{rows.length===1?"":"s"}</small></span><span>{collapsed ? "+" : "−"}</span></button>
    ...
    {activeSubTab === "g1" && <section style={styles.formSheet}><div style={styles.formTitle}><h2>Completion Certificate</h2><p>Form G1 for use with BS 7909</p></div><div style={styles.referenceRow}>{autoField("Certificate Reference No.","g1.reference",certificateReference)}</div>
      ...
      <FormPart title="Part 3: Essential inspection and tests"><div style={styles.checkGrid}><Check label="14. Visual inspection satisfactory" checked={document.g1.visualInspectionSatisfactory} disabled={locked} onChange={(value)=>patchG1({visualInspectionSatisfactory:value})}/><Check label="15. Polarity throughout satisfactory" checked={document.g1.polaritySatisfactory} disabled={locked} onChange={(value)=>patchG1({polaritySatisfactory:value})}/><Check label="16. Earth fault loop Z throughout satisfactory" checked={document.g1.earthFaultLoopSatisfactory} disabled={locked} onChange={(value)=>patchG1({earthFaultLoopSatisfactory:value})}/><Check label="17. RCD test buttons satisfactory" checked={document.g1.rcdButtonsSatisfactory} disabled={locked} onChange={(value)=>patchG1({rcdButtonsSatisfactory:value})}/><Check label="18. Evidence of formal inspection and test provided and satisfactory for electrical equipment." checked={document.g1.equipmentEvidenceSatisfactory} disabled={locked} onChange={(value)=>patchG1({equipmentEvidenceSatisfactory:value})}/>{autoField("19. Earth loop impedance of the supply (Ω)","g1.supplyZs",document.g1.supplyEarthLoopImpedanceOhms)}{autoField("20. Planned duration of this system","g1.duration",document.sharedInformation.operatingPeriod)}{autoField("21. Date to re-inspect and re-test this system","g1.reinspection",document.g1.reinspectionDate)}</div></FormPart>
